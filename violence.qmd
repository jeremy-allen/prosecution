...
ggplot() +
    ...
    geom_text(data = presidents, aes(x = year, y = 70, label = president), 
              vjust = -1.2, hjust = 0, angle = 90, size = 3, color = "black") +
...
The data is structured according to legal cases. A single defendant can be tried in multiple jurisdictions for the same incident. For example, a shooter can appear in a federal indictment and a state indictment for the same shooting. These would be two rows in the data. If we simply added the number of killed, we would overcount. 

How often does this happen? How much would we overcount if we simply summed the number killed?
```{r}
...
```
There are **`r nrow(dupes)`** defendants who were charged at multiple jurisdictional levels. If we simply summed the number killed, we would count **`r sum(dupe_counts$total_killed_if_summed, na.rm = TRUE)`** deaths instead of the actual **`r sum(dupe_counts$actual_killed, na.rm = TRUE)`** deaths. This would be  an overcount of **`r sum(dupe_counts$total_killed_if_summed, na.rm = TRUE) - sum(dupe_counts$actual_killed, na.rm = TRUE)`** deaths. So, I won't count duplicates.

Now, let's look at the number of deaths by ideological affiliation, avoiding double counting.
