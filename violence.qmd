...
Import the data, clean the column names, and filter for only the US.
```{r}
#| message: false
#| warning: false
...
There are *`r cases_not_political`* cases not politically motivated out of a total of *`r total_cases` cases*, which is *`r round((cases_not_political / total_cases) * 100, 2)`%*. So, we will exclude the non-politically motivated cases from our analysis.
...
plot1 <- ggplot(df_deaths, aes(x = reorder(ideology_simple, total_deaths), 
    ...
    coord_flip() +
    labs(
        title = "Total Deaths from Violent Crimes in the US",
        subtitle = "",
        x = "Ideological Affiliation",
