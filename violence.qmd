...
Out of our **`r nrow(violent_crimes)`** violent crime prosecutions, there are **`r nrow(dupes)`** defendants who were charged at multiple jurisdiction levels. If I simply summed the number killed from every row, I would mistakenly double count some deaths.  For these **`r nrow(dupes)`** defendants, I would get **`r sum(dupe_counts$total_killed_if_summed, na.rm = TRUE)`** deaths instead of the actual **`r sum(dupe_counts$actual_killed, na.rm = TRUE)`** deaths. This would be  an overcount of **`r sum(dupe_counts$total_killed_if_summed, na.rm = TRUE) - sum(dupe_counts$actual_killed, na.rm = TRUE)`** deaths.
